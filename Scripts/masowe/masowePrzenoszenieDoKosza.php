    ...
    foreach($tabela as $idW){
        ...
        while($wiersz=mysqli_fetch_array($zapytanie)){
            if($wiersz['idNadawcy']==$id){
                mysqli_query($polaczenie,"UPDATE Watki SET nadawcaKosz='1', nadawcaSpam='0' WHERE id=$idW;");
            }else if($wiersz['idOdbiorcy']==$id){
                mysqli_query($polaczenie,"UPDATE Watki SET odbiorcaKosz='1', odbiorcaSpam='0' WHERE id=$idW;");
            }
        }
    }
